...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        W2 = self.params["W2"]
        b2 = self.params["b2"]
        h1, fc_cache1 = affine_forward(X, W1, b1)
        out1, re_cache1 = relu_forward(h1)
        h2, fc_cache2 = affine_forward(out1, W2, b2)
        out2, re_cache2 = relu_forward(h2)
        scores = out2
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        dh2 = relu_backward(dout2, re_cache2)
        dout1, dw2, db2 = affine_backward(dh2, fc_cache2)
        dh1 = relu_backward(dout1, re_cache1)
        dx, dw1, db1 = affine_backward(dh1, fc_cache1)
        dw1 += self.reg*W1
        dw2 += self.reg*W2
        grads["W1"] = dw1
        grads["b1"] = db1
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for layer in range(self.num_layers):
            ...
            # batch nomarlization initialize
    ...
    def loss(self, X, y=None):
        ...
        for layer in range(self.num_layers):
            ...
            out, fc_cache = affine_forward(inputi, W, b)
            ...
            re_cache_list.append(re_cache)
            
            # use dropout
            if self.use_dropout:
                out, dr_cache = dropout_forward(out, self.dropout_param)
                dr_cache_list.append(dr_cache)    
            inputi = out
        ...
        W_sqrt_sum = 0
        for layer in range(self.num_layers):
            W_sqrt_sum += np.sum((self.params["W%d" % (layer + 1)])**2)
        loss = data_loss + 0.5*self.reg*W_sqrt_sum
        
        for layer in range(self.num_layers, 0, -1):
            # use dropout
            if self.use_dropout:
                dout = dropout_backward(dout, dr_cache_list[layer - 1])
            
            # use relu
            dout = relu_backward(dout, re_cache_list[layer - 1])
            ...
            dout, dw, db = affine_backward(dout, fc_cache_list[layer - 1])
